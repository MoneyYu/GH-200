---
image: https://images.credly.com/size/110x110/images/336eebfc-0ac3-4553-9a67-b402f491f185/azure-administrator-associate-600x600.png
tags: GH-200, Reference
GA: G-DXYJBX6BH8
---

# GH-200 Reference
:::success
Date: 20250904
Course ID: 86473
:::

:::info
Post Course Survey: [https://aka.ms/gh200survey](https://aka.ms/gh200survey)
:::

## Course Materials
[Course GH-200 English version](https://learn.microsoft.com/en-us/training/courses/gh-200t00)
[Course GH-200 简体中文版本](https://learn.microsoft.com/zh-cn/training/courses/gh-200t00)
[Course GH-200 正體中文版本](https://learn.microsoft.com/zh-tw/training/courses/gh-200t00)

## Infos
[Learner Experience Portal](https://esi.microsoft.com/)
[ESI Support](https://aka.ms/esisupport)

<!--
## Lab
### Skillable lab system
[ESI Labs](https://aka.ms/esilab)
> Training key: 4054FE03B45845DA
> > Only need to redeem once
> > Valid for 6 months

> Github CoPilot license provision may take between 10 and 40 minutes from the time of VM launch.

### Instruction
[GH-200 Labs EN](https://microsoftlearning.github.io/GH-200-Accelerate-app-development-by-using-GitHub-Copilot/)
[GH-200 Lab files (SampleApps.zip)](https://raw.githubusercontent.com/MicrosoftLearning/APL-2007-Accelerate-app-development-by-using-GitHub-Copilot/master/LearnModuleExercises/Downloads/SampleApps.zip)
-->

## Course Info
![01](https://mdcontent.yu.money/contents/f008fce23003844339a3ac100.zh-TW.png)

## Links
[GitHub’s plans](https://docs.github.com/en/get-started/learning-about-github/githubs-plans)

[GitHub Pricing Comparison](https://github.com/pricing#compare-features)

### Actions
[GitHub Actions](https://github.com/actions)

[GitHub Marketplace](https://github.com/marketplace)

[Events that trigger workflows](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows)

[Workflow syntax for GitHub Actions](https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-syntax)

[About billing for GitHub Actions](https://docs.github.com/en/actions/concepts/billing-and-usage)

[GitHub-hosted runners](https://docs.github.com/en/actions/concepts/runners/github-hosted-runners#supported-runners-and-hardware-resources)

[Self-hosted runners](https://docs.github.com/en/actions/concepts/runners/self-hosted-runners)

[Larger runners](https://docs.github.com/en/actions/concepts/runners/larger-runners#machine-sizes-for-larger-runners)

### Workflows
[Workflows](https://docs.github.com/en/enterprise-server@3.14/actions/concepts/workflows-and-actions/workflows)

[Store and share data with workflow artifacts](https://docs.github.com/zh/enterprise-cloud@latest/actions/tutorials/store-and-share-data)

[Variables reference](https://docs.github.com/en/actions/reference/workflows-and-actions/variables)

[Github context](https://docs.github.com/en/actions/reference/workflows-and-actions/contexts)

[Store information in variables](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/use-variables#default-environment-variables)

#### GitHub Apps
[About using GitHub Apps](https://docs.github.com/en/apps/using-github-apps/about-using-github-apps)

[Generating an installation access token for a GitHub App](https://docs.github.com/en/apps/creating-github-apps/authenticating-with-a-github-app/generating-an-installation-access-token-for-a-github-app)

### Deployment
[Evaluate expressions in workflows and actions](https://docs.github.com/en/actions/reference/workflows-and-actions/expressions)

[Using secrets in GitHub Actions](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/use-secrets)

[Configuring the retention period for GitHub Actions artifacts and logs in your organization](https://docs.github.com/en/organizations/managing-organization-settings/configuring-the-retention-period-for-github-actions-artifacts-and-logs-in-your-organization)

[Adding a workflow status badge](https://docs.github.com/en/actions/how-tos/monitor-workflows/add-a-status-badge)

### GitHub Scripts
[actions/github-script](https://github.com/actions/github-script)

[Octokit](https://github.com/octokit)

[octokit/rest.js](https://octokit.github.io/rest.js/v22/)

### GitHub Packages
[Introduction to GitHub Packages](https://docs.github.com/en/packages/learn-github-packages/introduction-to-github-packages)

[Releasing projects on GitHub](https://docs.github.com/en/repositories/releasing-projects-on-github)

## Exercises
- 01. [Exercise - Create and run a basic GitHub Actions workflow](https://learn.microsoft.com/en-us/training/modules/github-actions-automate-tasks/3-exercise-create-container-action)

- 02. [Exercise - Create the CI workflow on GitHub](https://learn.microsoft.com/en-us/training/modules/github-actions-ci/3-exercise-ci-workflow-github/?ns-enrollment-type=learningpath&ns-enrollment-id=learn.github-actions)

- 03. [Exercise - Create a workflow that deploys a web app to Azure](https://learn.microsoft.com/en-us/training/modules/github-actions-cd/3-create-workflow-deploy-azure/?ns-enrollment-type=learningpath&ns-enrollment-id=learn.github-actions)

- 04. [Exercise - Using GitHub Script in GitHub Actions](https://learn.microsoft.com/en-us/training/modules/automate-github-using-github-script/3-use-github-script/?ns-enrollment-type=learningpath&ns-enrollment-id=learn.github-actions)

- 05. [Exercise - Publish to a GitHub Packages registry](https://learn.microsoft.com/en-us/training/modules/github-actions-packages/3-exercise-github-packages-docker-registry/?ns-enrollment-type=learningpath&ns-enrollment-id=learn.github-actions-2)

- 06. [Exercise - Create a custom JavaScript GitHub action](https://learn.microsoft.com/en-us/training/modules/create-custom-github-actions/exercise-create-custom-action/?ns-enrollment-type=learningpath&ns-enrollment-id=learn.github-actions-2)

- 07. [Exercise - Use a repository secret in a GitHub Actions workflow](https://learn.microsoft.com/en-us/training/modules/manage-github-actions-enterprise/exercise/?ns-enrollment-type=learningpath&ns-enrollment-id=learn.github-actions-2)

## Demo GitHub
[GitHub: MoneyDemo](https://github.com/MoneyDemo)

## Exam
[GH-200 Exam Page](https://learn.microsoft.com/en-us/credentials/certifications/github-actions/?practice-assessment-type=certification)
[Practice Assessment](https://learn.microsoft.com/en-us/credentials/certifications/github-actions/practice/assessment?assessment-type=practice&assessmentId=1001&practice-assessment-type=certification)
[Exam duration and question types](https://learn.microsoft.com/en-us/credentials/support/exam-duration-exam-experience)
[Study Guide](https://learn.microsoft.com/en-us/credentials/certifications/resources/study-guides/gh-200)

[Exam Sandbox](https://aka.ms/examdemo)
[Microsoft Exam FAQ](https://docs.microsoft.com/en-us/learn/certifications/online-exams)
[Renew your Microsoft Certifications for free. Stay Microsoft Certified!](https://aka.ms/RenewYourCertVideo)
[Microsoft Learn for all role-based Microsoft Certification exams](https://techcommunity.microsoft.com/t5/microsoft-learn-blog/introducing-a-new-resource-for-all-role-based-microsoft/ba-p/3500870?WT.mc_id=general_Twitter-wwl)
<!--[](https://aka.ms/LxPExamDiscountVideo)-->


![exam](https://mdcontent.yu.money/contents/31b5b0d6ee886b9be732c5f03.png)

SVG: [exam.svg](https://mttcontent.yu.money/common/exam.svg)

## Contact
- Money Yu
- Mail: [Money.Yu@microsoft.com](mailto:Money.Yu@microsoft.com)
- LinkedIn: [@abc12207](https://www.linkedin.com/in/abc12207/)