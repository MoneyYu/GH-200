...
| 2 | **Attendee README** — HackMD reference page: module-grouped Links, one official-only Videos table with module labels, and a markmap mind map before Applied Skills; verify every link & video | [references/02-readme.md](references/02-readme.md) |
...
| 5 | **Validate & end-to-end test** — fmt/validate, AST parse, and (when repo rules permit) real apply + destroy | [references/05-validation.md](references/05-validation.md) |
...
6. **Verify every external link *semantically* before adding it** — check status **and** final URL
   ...
   to a generic hub/browse page. **Videos: official first-party channels ONLY** (Microsoft-owned;
   for GitHub courses, GitHub's official channel is also first-party), confirmed LIVE via
   YouTube oEmbed, presented in one flat table with module labels, no third-party and no padding. Reuse
...
7. **Python work uses a `uv`-managed virtual environment.** Create it with `uv venv .venv`.
   Commit only generated binary assets, not the generator; mark binary types in `.gitattributes`.
