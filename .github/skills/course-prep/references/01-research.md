...
- For PPT text extraction prefer a `uv`-managed Python venv with `python-pptx`, or any available
  extractor.
...
3. **Check localization.** Note whether the learning path and the lab repos have `zh-cn` / `zh-tw`
   versions. Add localized lab links to the attendee README only when those repositories actually
   exist and resolve to the correct exercises. Record unavailable localization in trainer research
   notes or `docs/`, not as an absence warning in the attendee README.
