...
static checks plus a **real** apply→destroy in Azure **when the target repo permits those
operations**. Repo-specific safety rules take precedence; if `apply` is forbidden, run every
permitted static/read-only check and record the missing end-to-end proof as a known limitation.
...
## End-to-end test (the real proof, when permitted)

`terraform apply` needs `az login`, a target subscription, and **model quota in the region** — it
**cannot** run in CI. Before continuing, check the target repo's Terraform instructions. If they
forbid `apply`, stop after `plan` and document the limitation. Otherwise, to validate the backup:
