...
real. Generate them **one-time** with Python (Pillow / reportlab / matplotlib) in a
**`uv`-managed venv** (`uv venv .venv`); commit only the resulting **binaries**, not the generator.
Mark every new binary type as `binary` in
